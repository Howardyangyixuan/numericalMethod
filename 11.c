...
//三角分解法求带状方阵的行列式
double sanjiaofenjie_daizhuang_det(Matrix_diag A)
{
    //原矩阵复制到A
    //Matrix_diag A;
    //A = Matrix_diagCopy(M);
...
void SanjiaoFenjie_daizhuang(Matrix_diag *a)
{
    const int n = a->dimension;
    const int r = a->r;
    const int s = a->s;
...
void After_Sanjiaofenjie_daizhuang(Matrix_diag *a, Vector B, Vector *x)
{
    const int n = a->dimension;
    const int r = a->r;
    const int s = a->s;
...
int main()
{
    ...
    det = sanjiaofenjie_daizhuang_det(A1);
    printf("det(A) = %.12e\n", det);
